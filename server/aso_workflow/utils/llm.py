...
def call_llm(
    ...
    **kwargs,
    ...
    If `use_web_search` is True, completions will use the model's native web search tool. `tools` will be ignored.
    If `tools` is provided and `use_web_search` is False, runs a tool-use loop:
...
def _accumulate_usage(usage_accumulator: dict, model: str, response: litellm.ModelResponse) -> None:
    ...
    Supports litellm.ModelResponse, Gemini GenerateContentResponse, and OpenAI Responses.
    If usage cannot be parsed, returns 0 for all usage tokens.
    """
    usage = {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0}
    if hasattr(response, "usage"):
        # handles litellm and OpenAI schemas
        ...
        total_tokens = getattr(usage, "total_token_count", None) or (input_tokens + output_tokens + thinking_tokens) or 0
        usage = {"input_tokens": input_tokens, "output_tokens": output_tokens, "total_tokens": total_tokens}
...
def _custom_parse_tool_call(msg: str) -> ChatCompletionMessageToolCall:
    """Parse a tool call from a message. Specifically for Qwen native tool calls."""
...
# ── LiteLLM-Style Tool Implementation ──────────────────────────────────────────────────────
